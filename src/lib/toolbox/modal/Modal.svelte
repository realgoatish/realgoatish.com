<script context="module">
	...
	export function prepareDOM() {
		...
		focusableElements = document.querySelectorAll(
			'a:not([tabindex="-1"]), button:not([tabindex="-1"]), input:not([tabindex="-1"]), select:not([tabindex="-1"]), textarea:not([tabindex="-1"]), details:not([tabindex="-1"]), [contenteditable]:not([tabindex="-1"])'
		);
...
<div class="overlay" bind:this={overlay}>
	<Imposter
		bind:imposterWrapperDiv={dialog}
